...
#------------------------------
    ...
    clawdata.inratx = [5,5,2,2]
    clawdata.inraty = [5,5,2,2]
    clawdata.inratt = [5,5,2,2]
...
def setgeo(rundata):
    ...
    dtopofile = os.path.join(topopath,'flumedoorsTXYZ_quadslow.txt')
    ...
    #geodata.regions.append([3,3,0.,1e10,-5,5,0,2])

    # == setgauges.data values ==
    geodata.gauges = []
    # for gauges append lines of the form  [gaugeno, x, y, t0, tf]
    geodata.gauges.append([2, 2.0*np.cos(31.*np.pi/180.0), 1.0, 0.0, 60e3])
    geodata.gauges.append([32, 32.0*np.cos(31.*np.pi/180.0), 1.0, 0.0, 60e3])
    geodata.gauges.append([66, 66.0*np.cos(31.*np.pi/180.0), 1.0, 0.0, 60e3])
