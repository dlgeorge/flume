...
def wallzeroCurved(X,Y):
    yind1 =  np.where((Y[:,0]>=-0.5)&(Y[:,0]<=0.0))[0]
    yind2 =  np.where((Y[:,0]>=2.0)&(Y[:,0]<=2.5))[0]
    xind  =  np.where((X[0,:]>=-15.0)&(X[0,:]<=82.5 + 1.2))[0]
    xhopperind = np.where((X[0,:]>=-15.0)&(X[0,:]<=0.0))[0]

    xcurveind = np.where((X[0,:]>=82.5+1.2)&(X[0,:]<=82.5+1.2+11.3833))[0]
    ycurveind = np.where((Y[:,0]>=-4.0)&(Y[:,0]<=4.0))[0]
    ...
    Z[np.ix_(ycurveind,xcurveind)] = curvedwall(X[np.ix_(ycurveind,xcurveind)],Y[np.ix_(ycurveind,xcurveind)],Z[np.ix_(ycurveind,xcurveind)])


    return Z

def curvedwall(X,Y,Z):
    ...
    for i in xrange(mx):
        ...
        for j in xrange(my):
            ...
            theta = np.arctan((x-xc)/(y-yc))

            if ((r>=10.0-1.5)&(r<=10.0-1.0)&(theta>=0.0)&(theta<=.85)):
                z = 1.6
            elif ((r>=10.0+1.0)&(r<=10.0+1.5)&(theta>=0.0)&(theta<=.85)):
                z = 4.0
            else:
                z = 0.0
...
gt.topo2writer(outfile,wallzeroCurved,xlower,xupper,ylower,yupper,nxpoints,nypoints)
