...
white2red_colormap = colormaps.make_colormap({0.0:white,
                                            1.0: red})
...
oso_debris_colormap_liquefaction = colormaps.make_colormap({0.0:white,
                                            #.33: blue,
                                            #.66: light_blue,
                                            1.0: dark_blue})
...
def Froude_number(current_data):
   drytol = getattr(current_data.user, 'drytol', drytol_default)
   q = current_data.q
   vnorm = velocity_norm(current_data)
   h = q[:,:,0]
   F = ma.masked_where(h<=drytol, vnorm/(np.sqrt(grav*h)))

   return F

def critical(current_data):
   drytol = getattr(current_data.user, 'drytol', drytol_default)
   q = current_data.q
   vnorm = velocity_norm(current_data)
   h = q[:,:,0]
